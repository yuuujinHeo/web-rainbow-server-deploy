...
const common_2 = __webpack_require__(8);
const path_1 = __webpack_require__(43);
const cobot_tcp_gateway_1 = __webpack_require__(179);
const cobot_tcp_service_1 = __webpack_require__(181);
const cobot_socket_service_1 = __webpack_require__(180);
const cobot_socket_mqtt_controller_1 = __webpack_require__(184);
...
exports.CobotSocketModule = CobotSocketModule = __decorate([
    (0, common_1.Module)({
        ...
        controllers: [cobot_socket_mqtt_controller_1.CobotSocketMqttController],
        providers: [cobot_socket_service_1.CobotSocketService, cobot_tcp_service_1.CobotTcpService, cobot_tcp_gateway_1.CobotTcpGateway],
    })
], CobotSocketModule);
...
var _a, _b, _c;
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.CobotTcpGateway = void 0;
const common_1 = __webpack_require__(8);
const common_2 = __webpack_require__(5);
const move_type_1 = __webpack_require__(99);
const control_type_1 = __webpack_require__(61);
...
const cobot_socket_service_1 = __webpack_require__(180);
const cobot_tcp_service_1 = __webpack_require__(181);
let CobotTcpGateway = class CobotTcpGateway {
    constructor(cobotService, tcpServerManager, configService) {
        this.cobotService = cobotService;
        this.tcpServerManager = tcpServerManager;
        this.configService = configService;
        this.clients = new Map();
        this.loggerService = common_1.LoggerService.get('gateway-tcp');
        this.init();
    }
    async init() {
        const cobotPortsEnv = this.configService.get('RRS_COBOT_PORTS');
        if (cobotPortsEnv) {
            const portStrings = cobotPortsEnv.split(',');
            const cobotPorts = [];
            portStrings.forEach((portStr) => {
                const port = parseInt(portStr.trim());
                if (!isNaN(port)) {
                    cobotPorts.push(port);
                }
            });
            if (cobotPorts.length > 0) {
                this.loggerService.info(`[Cobot] Starting Cobot TCP servers on ports: ${cobotPorts.join(', ')}`);
                for (const port of cobotPorts) {
                    try {
                        await this.TCP_Open(port);
                    }
                    catch (error) {
                        this.loggerService.error(`[Cobot] Error starting Cobot TCP server on port ${port}: ${(0, common_1.errorToJson)(error)}`);
                    }
                }
            }
            else {
                this.loggerService.warn('[Cobot] RRS_COBOT_PORTS configured but no valid ports found');
            }
        }
        else {
            this.loggerService.warn('[Cobot] RRS_COBOT_PORTS not configured - no TCP servers will be started');
        }
    }
    sendTCP(port, command) {
        try {
            if (this.clients[port]) {
                this.clients[port].send(command);
            }
            else {
                throw new rpc_code_exception_1.RpcCodeException(`${port} 포트에 연결된 클라이언트가 없습니다.`, constant_1.GrpcCode.NotFound);
            }
        }
        catch (error) {
            if (error instanceof microservices_1.RpcException)
                throw error;
            this.loggerService.error(`[Cobot] TCP Send Error: ${(0, common_1.errorToJson)(error)}`);
        }
    }
    ...
    setStatus(status) {
        this.cobotService.setStatus(status);
    }
    setMoveStatus(status) {
        this.cobotService.setMoveStatus(status);
    }
    onClientConnect(port, client) {
        try {
            if (this.clients[port]) {
                throw new rpc_code_exception_1.RpcCodeException(`${port} 포트에 이미 연결된 클라이언트가 있습니다.`, constant_1.GrpcCode.AlreadyExists);
            }
            else {
                this.clients[port] = client;
                this.loggerService.info(`[Cobot] TCP Client Connected: ${client.getId()}`);
                this.cobotService.cobotConnect(port);
            }
        }
        catch (error) {
            if (error instanceof microservices_1.RpcException)
                throw error;
            this.loggerService.error(`[Cobot] TCP Client Connect Error: ${(0, common_1.errorToJson)(error)}`);
        }
    }
    onClientDisconnect(port, client) {
        try {
            if (this.clients[port]) {
                delete this.clients[port];
                this.loggerService.info(`[Cobot] TCP Client Disconnected: ${client.getId()}`);
                this.cobotService.cobotDisconnect(port);
            }
            else {
                throw new rpc_code_exception_1.RpcCodeException(`${port} 포트에 연결된 클라이언트가 없습니다.`, constant_1.GrpcCode.NotFound);
            }
            ...
            if (error instanceof microservices_1.RpcException)
                throw error;
            this.loggerService.error(`[Cobot] TCP Client Disconnect Error: ${(0, common_1.errorToJson)(error)}`);
        }
    }
    async onDataReceived(port, client, data) {
        this.loggerService.info(`[Cobot] TCP Data received from ${port}: ${data}`);
        try {
            ...
            if (command === 'req_status') {
                try {
                    if (!param) {
                        return 'Error: No param specified';
                    }
                    const statusData = this.cobotService.getStatus(param);
                    this.loggerService.debug(`[Cobot] TCP Request Status: ${param} -> ${statusData}`);
                    return statusData;
                }
                catch (error) {
                    this.loggerService.error(`[Cobot] TCP Request Status Error: ${(0, common_1.errorToJson)(error)}`);
                    return `Error: ${error.message}`;
                }
            }
            else if (command === 'response') {
                this.loggerService.debug(`[Cobot] TCP Response ${port} : ${param}`);
                this.cobotService.cobotResponse(port, param);
            }
            else if (command === 'move') {
                if (!param) {
                    this.loggerService.error(`[Cobot] TCP Move Error: No param specified`);
                    return 'Error: Move command requires a parameter';
                }
                this.loggerService.debug(`[Cobot] TCP Move Request: ${param}`);
                if (param === 'stop') {
                    const resp = await this.cobotService.moveRequest({ command: move_type_1.MoveCommand.moveStop });
                    return resp;
                }
                else if (param === 'pause') {
                    const resp = await this.cobotService.moveRequest({ command: move_type_1.MoveCommand.movePause });
                    return resp;
                }
                else if (param === 'resume') {
                    const resp = await this.cobotService.moveRequest({ command: move_type_1.MoveCommand.moveResume });
                    return resp;
                }
                else if (param) {
                    const resp = await this.cobotService.moveRequest({
                        command: 'goal',
                        goalId: param,
                        method: 'pp',
                        preset: 0,
                    ...
                    this.loggerService.error(`[Cobot] TCP Move Error: Unknown move command: ${param}`);
                    return 'Error: Unknown move command';
                }
            }
            else if (command === 'dock') {
                this.loggerService.debug(`[Cobot] TCP Dock Request`);
                const resp = await this.cobotService.controlRequest({ command: control_type_1.ControlCommand.dockStart });
                return resp;
            }
            else if (command === 'undock') {
                this.loggerService.debug(`[Cobot] TCP UnDock Request`);
                const resp = await this.cobotService.controlRequest({ command: control_type_1.ControlCommand.undockStart });
                return resp;
            }
            else if (command === 'localization') {
                if (!param) {
                    this.loggerService.error(`[Cobot] TCP Localization Error: No param specified`);
                    return 'Error: Localization command requires a parameter';
                }
                this.loggerService.debug(`[Cobot] TCP Localization Request: ${param}`);
                let sendData;
                if (param === 'semiautoinit') {
                    sendData = { command: localization_type_1.LocalizationCommand.semiAutoInit };
                }
                else if (param === 'autoinit') {
                    sendData = { command: localization_type_1.LocalizationCommand.autoInit };
                }
                else if (param === 'start') {
                    sendData = { command: localization_type_1.LocalizationCommand.start };
                }
                else if (param === 'stop') {
                    sendData = { command: localization_type_1.LocalizationCommand.stop };
                }
                    ...
                    this.loggerService.error(`[Cobot] TCP Localization Error: Unknown localization command: ${param}`);
                    return 'Error: Unknown localization command';
                }
                const resp = await this.cobotService.localzationRequest(sendData);
                return resp;
            }
            else if (command === 'mapload') {
                if (!param) {
                    this.loggerService.error(`[Cobot] TCP Map Load Error: No param specified`);
                    return 'Error: Map load command requires a map name';
                }
                this.loggerService.debug(`[Cobot] TCP Map Load Request: ${param}`);
                const resp = await this.cobotService.loadRequest({ command: map_command_domain_1.MapCommand.loadMap, mapName: param });
                return resp;
            }
            else {
                this.loggerService.error(`[Cobot] TCP Unknown Command: ${command}`);
                return `Error: Unknown command`;
            }
        }
        catch (error) {
            this.loggerService.error(`[Cobot] TCP Data Parse from ${port}: ${(0, common_1.errorToJson)(error)}`);
            return 'Error: Parsing Data';
        }
    }
    async TCP_Open(port) {
        return await this.tcpServerManager.startServer(port, this);
    }
    async closeAllServers() {
        await this.tcpServerManager.stopAllServers();
    }
    getServerStatus() {
        return this.tcpServerManager.getServerStatus();
    }
};
exports.CobotTcpGateway = CobotTcpGateway;
exports.CobotTcpGateway = CobotTcpGateway = __decorate([
    (0, common_2.Injectable)(),
    __metadata("design:paramtypes", [typeof (_a = typeof cobot_socket_service_1.CobotSocketService !== "undefined" && cobot_socket_service_1.CobotSocketService) === "function" ? _a : Object, typeof (_b = typeof cobot_tcp_service_1.CobotTcpService !== "undefined" && cobot_tcp_service_1.CobotTcpService) === "function" ? _b : Object, typeof (_c = typeof config_1.ConfigService !== "undefined" && config_1.ConfigService) === "function" ? _c : Object])
], CobotTcpGateway);
...
var _a, _b, _c, _d;
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.CobotSocketService = void 0;
const common_1 = __webpack_require__(8);
const constant_1 = __webpack_require__(65);
const common_2 = __webpack_require__(5);
const microservices_1 = __webpack_require__(3);
const rxjs_1 = __webpack_require__(31);
let CobotSocketService = class CobotSocketService {
    constructor(moveMicroservice, localizationMicroservice, controlMicroservice, mqttMicroservice) {
        this.moveMicroservice = moveMicroservice;
        this.localizationMicroservice = localizationMicroservice;
        this.controlMicroservice = controlMicroservice;
        this.mqttMicroservice = mqttMicroservice;
        this.loggerService = common_1.LoggerService.get('gateway');
        this.con_slamnav = false;
    }
    onModuleInit() {
        this.moveService = this.moveMicroservice.getService('MoveGrpcService');
        this.localizationService =
    ...
    cobotConnect(port) {
        this.mqttMicroservice.emit('con:cobot', port.toString());
    }
    cobotDisconnect(port) {
        this.mqttMicroservice.emit('discon:cobot', port.toString());
    }
    async moveRequest(request) {
        try {
            const resp = await (0, rxjs_1.lastValueFrom)(this.moveService.moveCommand(request));
            return 'accept';
        }
        catch (error) {
            this.loggerService.error(`[Cobot] moveRequest : ${(0, common_1.errorToJson)(error)}`);
            return `Error: ${(0, common_1.errorToJson)(error)}`;
        }
    }
    async controlRequest(request) {
        try {
            const resp = await (0, rxjs_1.lastValueFrom)(this.controlService.workControl(request));
            return 'accept';
        }
        catch (error) {
            this.loggerService.error(`[Cobot] controlRequest : ${(0, common_1.errorToJson)(error)}`);
            return `Error: ${(0, common_1.errorToJson)(error)}`;
        }
    }
    async localzationRequest(request) {
        try {
            const resp = await (0, rxjs_1.lastValueFrom)(this.localizationService.init(request));
            return 'accept';
        }
        catch (error) {
            this.loggerService.error(`[Cobot] localizationRequest : ${(0, common_1.errorToJson)(error)}`);
            return `Error: ${(0, common_1.errorToJson)(error)}`;
        }
    }
    async loadRequest(request) {
        try {
            const resp = await (0, rxjs_1.lastValueFrom)(this.mapService.load(request));
            return 'accept';
        }
        catch (error) {
            this.loggerService.error(`[Cobot] loadRequest : ${(0, common_1.errorToJson)(error)}`);
            return `Error: ${(0, common_1.errorToJson)(error)}`;
        }
    }
    cobotResponse(port, command) {
        this.mqttMicroservice.emit('cobotResponse', { cobotPort: port, command: command });
    }
    getStatus(param) {
        switch (param) {
            case 'slamnav':
                return this.con_slamnav ? 'connected' : 'disconnected';
            case 'pose':
                return `{${this.moveStatus.pose.x},${this.moveStatus.pose.y},${this.moveStatus.pose.rz}}`;
            case 'map':
                return this.status.map.map_name;
            case 'localization':
                return this.status.robot_state.localization;
            case 'charge':
                return this.status.robot_state.charge;
            case 'dock':
                return this.status.robot_state.dock;
            case 'auto_move':
                return this.moveStatus.move_state.auto_move;
            case 'path':
                return this.moveStatus.move_state.path;
            case 'obs':
                return this.moveStatus.move_state.obs;
            case 'cur_node_id':
                return this.moveStatus.cur_node.id;
            case 'cur_node_pose':
                return `{${this.moveStatus.cur_node.x},${this.moveStatus.cur_node.y},${this.moveStatus.cur_node.rz}}`;
            case 'goal_node_id':
                return this.moveStatus.goal_node.id;
            case 'goal_node_pose':
                return `{${this.moveStatus.goal_node.x},${this.moveStatus.goal_node.y},${this.moveStatus.goal_node.rz}}`;
            case 'goal_node_state':
                return this.moveStatus.goal_node.state;
            case 'battery':
                return `{${this.status.power.bat_in},${this.status.power.bat_out},${this.status.power.bat_current}}`;
            default:
                throw new Error(`Error unknown param name (${param})`);
        }
    }
};
exports.CobotSocketService = CobotSocketService;
exports.CobotSocketService = CobotSocketService = __decorate([
    (0, common_2.Injectable)(),
    __param(0, (0, common_2.Inject)(constant_1.MOVE_SERVICE)),
    __param(1, (0, common_2.Inject)(constant_1.LOCALIZATION_SERVICE)),
    __param(2, (0, common_2.Inject)(constant_1.CONTROL_SERVICE)),
    __param(3, (0, common_2.Inject)(constant_1.MQTT_BROKER)),
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.CobotTcpService = void 0;
const common_1 = __webpack_require__(5);
const common_2 = __webpack_require__(8);
const cobot_tcp_output_port_1 = __webpack_require__(182);
let CobotTcpService = class CobotTcpService {
    constructor() {
        this.servers = new Map();
        this.loggerService = common_2.LoggerService.get('tcp-server-manager');
    }
    async startServer(port, handler) {
        if (this.isServerRunning(port)) {
            this.loggerService.warn(`[Cobot] Server already running on port: ${port}`);
            return false;
        }
        try {
            const server = cobot_tcp_output_port_1.CobotTcpPort.createServer(port, handler);
            await server.start(port);
            this.servers.set(port, server);
            this.loggerService.info(`[Cobot] Server started on port: ${port}`);
            return true;
        }
        catch (error) {
            this.loggerService.error(`[Cobot] Failed to start server on port ${port}: ${error.message}`);
            return false;
        }
    }
    async stopServer(port) {
        ...
        if (!server) {
            this.loggerService.warn(`[Cobot] No server running on port: ${port}`);
            return false;
        }
        try {
            await server.stop();
            this.servers.delete(port);
            this.loggerService.info(`[Cobot] Server stopped on port: ${port}`);
            return true;
        }
        catch (error) {
            this.loggerService.error(`[Cobot] Failed to stop server on port ${port}: ${error.message}`);
            return false;
        }
    }
    async stopAllServers() {
        const ports = Array.from(this.servers.keys());
        this.loggerService.info(`[Cobot] Stopping ${ports.length} servers...`);
        for (const port of ports) {
            await this.stopServer(port);
        }
        this.loggerService.info(`[Cobot] All servers stopped`);
    }
    ...
    getServerStatus() {
        const runningPorts = this.getRunningPorts();
        return {
            totalServers: runningPorts.length,
            runningPorts: runningPorts,
            isRunning: (port) => this.isServerRunning(port),
        };
    }
    getServer(port) {
...
exports.CobotTcpService = CobotTcpService;
exports.CobotTcpService = CobotTcpService = __decorate([
    (0, common_1.Injectable)()
], CobotTcpService);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.CobotTcpPort = void 0;
class CobotTcpPort {
    static createServer(port, handler) {
        return new TcpServerImpl(port, handler);
    }
}
exports.CobotTcpPort = CobotTcpPort;
class TcpServerImpl {
    ...
    async start(port) {
        const { createServer } = await Promise.resolve().then(() => __webpack_require__(183));
        this.server = createServer((socket) => {
            const client = new TcpClientImpl(socket);
            this.handler.onClientConnect(port, client);
            this.clients.set(client.getId(), client);
            socket.on('data', async (data) => {
                try {
                    const response = await this.handler.onDataReceived(port, client, data.toString());
                    if (response) {
            ...
            socket.on('end', () => {
                this.handler.onClientDisconnect(port, client);
...
var _a, _b, _c, _d, _e, _f, _g, _h;
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.CobotSocketMqttController = void 0;
const common_1 = __webpack_require__(8);
const common_2 = __webpack_require__(5);
const microservices_1 = __webpack_require__(3);
const status_type_1 = __webpack_require__(149);
...
const cobot_tcp_gateway_1 = __webpack_require__(179);
const cobot_socket_service_1 = __webpack_require__(180);
let CobotSocketMqttController = class CobotSocketMqttController {
    constructor(cobotService, tcpGateway) {
        this.cobotService = cobotService;
        this.tcpGateway = tcpGateway;
        this.loggerService = common_1.LoggerService.get('gateway');
    }
    connectSlamnav() {
        this.loggerService.info('[COBOT] Connect Slamnav');
        this.cobotService.conSlamnav();
    }
    disconnectSlamnav() {
        this.loggerService.info('[COBOT] Disconnect Slamnav');
        this.cobotService.disconSlamnav();
    }
    getStatus(status) {
        this.tcpGateway.setStatus(status);
    }
    getMoveStatus(status) {
        this.tcpGateway.setMoveStatus(status);
    }
    getMoveResponse(response) {
        this.tcpGateway.moveResponse(response);
    }
    getLocalizationResponse(response) {
        this.tcpGateway.localizationResponse(response);
    }
    getLoadResponse(response) {
        this.tcpGateway.loadResponse(response);
    }
    getControlResponse(response) {
        this.tcpGateway.controlResponse(response);
    }
};
exports.CobotSocketMqttController = CobotSocketMqttController;
__decorate([
    (0, microservices_1.MessagePattern)('con:slamnav'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], CobotSocketMqttController.prototype, "connectSlamnav", null);
__decorate([
    (0, microservices_1.MessagePattern)('discon:slamnav'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], CobotSocketMqttController.prototype, "disconnectSlamnav", null);
__decorate([
    (0, microservices_1.MessagePattern)('status'),
    __metadata("design:type", Function),
    ...
    __metadata("design:returntype", void 0)
], CobotSocketMqttController.prototype, "getStatus", null);
__decorate([
    (0, microservices_1.MessagePattern)('moveStatus'),
    __metadata("design:type", Function),
    ...
    __metadata("design:returntype", void 0)
], CobotSocketMqttController.prototype, "getMoveStatus", null);
__decorate([
    (0, microservices_1.MessagePattern)('moveResponse'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [typeof (_e = typeof move_dto_1.MoveResponseSlamnav !== "undefined" && move_dto_1.MoveResponseSlamnav) === "function" ? _e : Object]),
    __metadata("design:returntype", void 0)
], CobotSocketMqttController.prototype, "getMoveResponse", null);
__decorate([
    (0, microservices_1.MessagePattern)('localizationResponse'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [typeof (_f = typeof localization_dto_1.LocalizationResponseSlamnav !== "undefined" && localization_dto_1.LocalizationResponseSlamnav) === "function" ? _f : Object]),
    __metadata("design:returntype", void 0)
], CobotSocketMqttController.prototype, "getLocalizationResponse", null);
__decorate([
    (0, microservices_1.MessagePattern)('loadResponse'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [typeof (_g = typeof load_dto_1.LoadResponseSlamnav !== "undefined" && load_dto_1.LoadResponseSlamnav) === "function" ? _g : Object]),
    __metadata("design:returntype", void 0)
], CobotSocketMqttController.prototype, "getLoadResponse", null);
__decorate([
    (0, microservices_1.MessagePattern)('controlResponse'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [typeof (_h = typeof control_dto_1.ControlResponseSlamnav !== "undefined" && control_dto_1.ControlResponseSlamnav) === "function" ? _h : Object]),
    __metadata("design:returntype", void 0)
], CobotSocketMqttController.prototype, "getControlResponse", null);
exports.CobotSocketMqttController = CobotSocketMqttController = __decorate([
    (0, common_2.Controller)(),
    __metadata("design:paramtypes", [typeof (_a = typeof cobot_socket_service_1.CobotSocketService !== "undefined" && cobot_socket_service_1.CobotSocketService) === "function" ? _a : Object, typeof (_b = typeof cobot_tcp_gateway_1.CobotTcpGateway !== "undefined" && cobot_tcp_gateway_1.CobotTcpGateway) === "function" ? _b : Object])
], CobotSocketMqttController);
...
const cobot_socket_module_1 = __webpack_require__(178);
