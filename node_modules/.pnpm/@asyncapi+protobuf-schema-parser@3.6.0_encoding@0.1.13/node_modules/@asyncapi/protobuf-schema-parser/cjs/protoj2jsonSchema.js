...
class Proto2JsonSchema {
    ...
    fetch(filename, parentFilename, weak) {
        ...
        if (bundledFilename in protobuf.common) {
            // @ts-ignore
            this.process(bundledFilename, protobuf.common[bundledFilename]);
            return;
        }
        if (bundledFilename in google_types_1.googleProtoTypes) {
            this.process(bundledFilename, google_types_1.googleProtoTypes[bundledFilename]);
            return;
        }
        const resolvedFilename = pathUtils_1.Path.resolve(bundledFilename, parentFilename);
        if (!weak) {
    ...
    isProto3Required(field) {
        var _a;
        if ((0, protovalidate_1.isOptional)(field)) {
            return false;
        }
    ...
    compileMessage(item, stack) {
        ...
        for (const fieldName in item.fields) {
            ...
            if (field.repeated) {
                ...
                (0, protovalidate_1.visit)(properties[field.name], field);
            }
            else {
        ...
        for (const oneOfItem of item.oneofsArray) {
            ...
            const oneOf = properties[oneOfItem.name]['oneOf'];
    ...
    compileField(field, parentItem, stack) {
        ...
        if (!field.repeated) {
            ...
            (0, protovalidate_1.visit)(obj, field);
        }
        const desc = this.extractDescription(field.comment);
        if (desc !== null && desc.length > 0) {
            if (obj.description) {
